...
import { getCompletionKeyword, outputMatchesTaskCompletion } from '../goReader.js';
...
export async function workerAgentNode(state) {
  ...
  const scriptRule = [
    '작업 규칙:',
    '- 실행용 스크립트 파일을 새로 만들어야 하면 반드시 script/ 디렉터리 아래에 생성하세요.',
    '- 루트나 다른 임의 위치에 실행 스크립트를 만들지 마세요.',
    '',
  ].join('\n');
  ...
  const treatIterationAsCompletion = !isInfiniteContextMode;
